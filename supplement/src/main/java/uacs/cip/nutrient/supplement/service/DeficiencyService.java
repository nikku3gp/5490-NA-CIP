...
import uacs.cip.nutrient.supplement.repository.DeficiencyRepository;
...
public class DeficiencyService {
    @Autowired
    public DeficiencyRepository deficiencyRepository;
    @Autowired
    public NutrientGoalRepository nutrientGoalRepository;
    @Autowired
    public LogSupplementRepository logSupplementRepository;

    public List<Deficiency> getDeficiencyFroDate(LocalDate date){
        ...
        for(NutrientGoal goal:goals){
            ...
            if(consumed < target){
                ...
                alerts.add(new Deficiency(nutrient, consumed, target, deficit));
            }
        }
        return alerts;
    }
